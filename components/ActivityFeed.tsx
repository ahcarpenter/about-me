...
export default function ActivityFeed({
  ...
  githubItems: GithubFeedItem[];
}) {
  const [filter, setFilter] = useState<Source | "all">("all");

  const items = useMemo(() => {
    const github: FeedItem[] = githubItems.map((i) => ({ ...i, source: "github" as const }));
    ...
    return [...github, ...substackItems, ...linkedinItems, ...readingItems]
      .filter((i) => i.date)
      ...
      <ul className="card mt-5 divide-y divide-line overflow-hidden">
        {visible.length === 0 && (
          <li className="px-5 py-8 text-center text-sm text-muted">
            Nothing here yet
            {filter === "github" && githubItems.length === 0 ? (
              <>
                {" — "}
                <a
                  href={site.githubUrl}
